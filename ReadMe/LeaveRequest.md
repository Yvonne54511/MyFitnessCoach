...
```
Users (既有)
  ├── 1:1 → Instructors  (教練業務：時薪、照片、CancelCount)
  ├── 1:1 → Employees    (HR 人事：部門、主管、請假)
  └── 1:1 → Members      (會員：身體數據、飲食)
...
### 1. Employees vs. Instructors 的關係（已決策：獨立共存）
`Instructors` 負責**業務排程**（預約課程、`CancelCount`），`Employees` 負責**HR 人事**（請假審核、部門歸屬），兩者職責不同，獨立共存是正確設計。

```
取消預約（CancelCount）≠ 請假（LeaveRequest）
```

- 教練取消某堂預約課程 → 更新 `Instructors.CancelCount`，與請假無關
- 教練提交正式假單 → 走 `LeaveRequests` 審核流程
- 若請假核准後需自動取消期間預約 → 在**應用服務層**以事件觸發處理，DB Schema 層兩張表保持獨立

兩張表都透過 `UserId` 關聯 `Users`，不需要額外橋接欄位。
